...
pub trait ComponentEncode {
    ...
    fn post_encode(
        ...
        _working_value: &Self::WorkingValue,
    ) -> Result<(), EncodeError> {
        Ok(())
    }
}
...
pub trait PrimitiveComponent: Default {
    ...
    fn do_decode(decoder: &Decoder, offset: u32) -> Result<Self, DecodeError>
    where
        Self: Sized;
}
...
impl<T: ComponentEncode> ComponentEncode for Option<T> {
    ...
    fn vtable_encode(
        ...
        match (self, working_value) {
            ...
            _ => Err(EncodeError::InvalidStructure),
        }
    }
    fn post_encode(
        &self,
...
impl<T: ComponentDecode> ComponentDecode for Option<T> {
        ...
        match value {
            ...
            _ => {
                let (working_value, next_offset) =
                    T::vtable_decode(decoder, table_start, vtable_entry)?;
    ...
    fn vector_len_decode(
        ...
        match working_value {
            Some(working_value_inner) => T::vector_len_decode(decoder, working_value_inner),
            None => Ok(0),
        }
    }
    fn vector_value_decode(
    ...
    {
        match working_value {
            Some(working_value_inner) => Ok(Some(T::vector_value_decode(
                decoder,
                working_value_inner,
                idx,
            )?)),
